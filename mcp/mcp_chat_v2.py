...
class NarrativeSynthesizer:
    ...
    async def generate(
        ...
        artifact_summary = ""
        if artifacts:
            artifact_types = {"map": [], "chart": [], "table": []}
            for artifact in artifacts:
                if artifact.type in artifact_types:
                    title = artifact.title or f"{artifact.type} visualization"
                    artifact_types[artifact.type].append(title)

            artifact_parts = []
            if artifact_types["map"]:
                artifact_parts.append(f"{len(artifact_types['map'])} map(s): {', '.join(artifact_types['map'])}")
            if artifact_types["chart"]:
                artifact_parts.append(f"{len(artifact_types['chart'])} chart(s): {', '.join(artifact_types['chart'])}")
            if artifact_types["table"]:
                artifact_parts.append(f"{len(artifact_types['table'])} table(s): {', '.join(artifact_types['table'])}")

            if artifact_parts:
                artifact_summary = "\n\nAVAILABLE VISUALIZATIONS:\nThe following visualizations have been generated and will be displayed to the user:\n- " + "\n- ".join(artifact_parts)
        ...
        prompt = """CONTEXT:
            ...
            VISUALS & STRUCTURED DATA:
            - Maps, charts, and tables have been generated by the system and are listed in the "AVAILABLE VISUALIZATIONS" section below.
            - Reference these visualizations naturally in your narrative when they support the evidence (e.g., "The map shows deforestation patterns..." or "As visualized in the chart...").
            - Do NOT say you cannot generate visualizations if they are listed as available.
            - If no visualizations are listed, explain the evidence without claiming visuals are unavailable.
...
class SimpleOrchestrator:
    ...
    async def _run_pipeline(
        ...
        *,
        ...
        # Collect all artifacts to inform narrative generation
        all_artifacts: List[ArtifactPayload] = []
        for response in responses:
            all_artifacts.extend(response.artifacts)
        print(
            f"[KGDEBUG] collected {len(all_artifacts)} artifacts for narrative context",
            flush=True,
        )
        ...
        try:
            narrative_result = await self._narrative.generate(
                query, ordered_evidences, ordered_ids, artifacts=all_artifacts
            )
            print(
